...
def simple_model():
    model = nn.Sequential(
        nn.Conv1d(in_channels=1, out_channels=64,
                  kernel_size=50, stride=6, bias=False),
...
class SimpleModel:
    ...
    def train(self, minibatch_fn):
        ...
        for x, y, w, sl, re in minibatch_fn:
            # shape(batch_size * seq_length, in_channels, input_length)
            ...
            tmp_preds = np.reshape(np.argmax(y_pred.cpu().detach().numpy(
            ), axis=1), (self.config["batch_size"], self.config["seq_length"]))
            tmp_trues = np.reshape(y.cpu().detach().numpy(
        ...
        f1_score = skmetrics.f1_score(
            y_true=trues, y_pred=preds, average="macro")
        cm = skmetrics.confusion_matrix(
            y_true=trues, y_pred=preds, labels=[0, 1, 2, 3, 4])
